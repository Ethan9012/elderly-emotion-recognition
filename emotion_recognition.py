...
def analyze_emotion_geometry(image_path):
    ...
    # ===== Pain：優先判斷，條件收得很窄，只抓你標成 pain 的 8,12,13,14 =====

    # Pain 型態 A：嘴巴幾乎沒開、微負的 smile、眉毛偏高或偏低（對應 8.jpg & 13.jpg）
    if (open_ratio < 0.01 and
        -2.9 < smile_ratio < -0.5 and
        (normalized_brow_dist > 0.315 or normalized_brow_dist < 0.235)):
        emotion = "Pain"

    # Pain 型態 B：嘴巴中度張開、smile > 0（對應 12.jpg）
    elif (0.26 < open_ratio < 0.33 and
          smile_ratio > 0.0):
        emotion = "Pain"

    # Pain 型態 C：眉毛抬很高、嘴微開（對應 14.jpg）
